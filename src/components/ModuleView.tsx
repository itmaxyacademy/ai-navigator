...
export const ModuleView: React.FC<ModuleViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12 relative">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-slate-900 border border-slate-700/80 p-3.5 sm:p-4 rounded-3xl shadow-xl">
        <button
          onClick={onBackToPath}
          className="px-4 py-2 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white text-xs font-bold flex items-center gap-2 border border-slate-700 transition-all cursor-pointer shrink-0 shadow-sm"
        >
          <ArrowLeft className="w-4 h-4 text-amber-400" /> Kembali <span className="hidden sm:inline">ke Peta Jalan</span>
        </button>

        {/* Section Tabs Stepper */}
        <div className="flex items-center gap-1.5 bg-slate-950/80 p-1.5 rounded-2xl border border-slate-800 w-full md:w-auto overflow-x-auto">
          <button
