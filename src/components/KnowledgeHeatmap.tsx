...
export const KnowledgeHeatmap: React.FC<KnowledgeHeatmapProps> = ({
  ...
  return (
    <div className="bg-white border-slate-200 text-slate-900 dark:bg-white dark:bg-white/90 dark:bg-slate-900/90 dark:border-slate-200 dark:border-slate-800 dark:text-slate-900 dark:text-white border rounded-3xl p-6 shadow-2xl space-y-6 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-200 dark:border-slate-800 pb-5">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-slate-900 dark:text-white flex items-center gap-2">
          ...
          <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400 text-xs leading-relaxed max-w-xl">
            Memetakan modul yang sering dipelajari ulang atau yang memerlukan latihan ekstra berdasarkan perolehan skor kuis.
          </p>
        </div>

        {/* Legend Indicator */}
        <div className="bg-slate-50 dark:bg-slate-100 dark:bg-slate-950/80 p-3 rounded-2xl border border-slate-200 dark:border-slate-200 dark:border-slate-800/80 text-xs space-y-2 shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10">
        <div className="bg-slate-50 dark:bg-slate-100 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 space-y-1">
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-500 dark:text-slate-400">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-2 relative z-10">
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-200 dark:border-slate-800 text-xs">
          <button
            onClick={() => setActiveFilter('all')}
            className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
              activeFilter === 'all'
                ? 'bg-indigo-600 text-slate-900 dark:text-white shadow'
                : 'text-slate-600 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-700 dark:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1.5 ${
              activeFilter === 'struggling'
                ? 'bg-rose-600 text-slate-900 dark:text-white shadow'
                : 'text-slate-600 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-700 dark:text-slate-200'
            }`}
          >
        ...
        <div className="text-[11px] text-slate-500 dark:text-slate-500 dark:text-slate-400 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5 relative z-10">
        {filteredData.map((node) => {
          ...
          let borderColor = 'border-slate-200 dark:border-slate-200 dark:border-slate-800';
          if (isStruggling) {
          ...
          return (
            <div
              ...
              className={`bg-slate-50 dark:bg-slate-100 dark:bg-slate-950/80 border ${borderColor} rounded-2xl p-4 transition-all duration-200 hover:-translate-y-1 hover:shadow-xl cursor-pointer flex flex-col justify-between gap-3 group relative overflow-hidden`}
            >
              ...
              <div className="flex items-start justify-between gap-2 pt-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-amber-400 transition-colors truncate">
                    {node.title}
                  </h3>
                </div>
              ...
              {/* Subtitle / Badge */}
              <p className="text-[11px] text-slate-500 dark:text-slate-500 dark:text-slate-400 truncate">
                {node.badge || node.subtitle || 'Modul AI'}
              </p>

              {/* Footer Row (Score + Revisit chip) */}
              <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800/80">
                <div>
                  {node.isCompleted ? (
                    <span
                ...
                <span className="px-2 py-0.5 rounded-lg bg-slate-200 dark:bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-600 dark:text-slate-300 text-[10px] font-bold flex items-center gap-1">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 pt-2">
                <button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-extrabold py-3 px-4 rounded-2xl shadow-xl flex items-center justify-center gap-2 text-xs transition-transform hover:scale-[1.02]"
                >
