...
export const PromptingGuideSection: React.FC<PromptingGuideSectionProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Top Banner Header */}
      <div className="relative overflow-hidden rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-2xl">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          {guide.tips.map((tip, index) => (
            <div
              key={index}
              className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 p-6 space-y-5 hover:border-indigo-500/40 transition-all shadow-xl relative"
            >
              {/* Tip Header */}
              ...
              <div className="space-y-2">
                ...
                <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 font-mono text-xs text-indigo-200/90 whitespace-pre-wrap leading-relaxed relative">
                  {tip.promptExample}
                </div>
              </div>

              {/* Pro Tip Callout Box */}
              <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-amber-800/40 flex items-start gap-3">
                <Lightbulb className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                <div className="space-y-0.5 text-xs">
                  <span className="font-bold text-amber-300">Pro Tip Pakar: </span>
                  <span className="text-slate-600 dark:text-slate-300 leading-relaxed">{tip.proTip}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* Bottom CTA to Quiz */}
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-indigo-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="space-y-1 text-center sm:text-left">
          <h4 className="text-base font-bold text-slate-900 dark:text-white">Sudah Menguasai Teknik Prompting {guide.llmName}?</h4>
          <p className="text-xs text-slate-600 dark:text-slate-300">Uji pemahaman Anda melalui kuis interaktif untuk mendapatkan skor & badge!</p>
        </div>
        <button
          onClick={onAdvanceToQuiz}
